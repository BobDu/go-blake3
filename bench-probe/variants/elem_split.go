...
func Compress(
	...
	var s [16]uint32
	s[0], s[1], s[2], s[3] = chain[0], chain[1], chain[2], chain[3]
	s[4], s[5], s[6], s[7] = chain[4], chain[5], chain[6], chain[7]
	s[8], s[9], s[10], s[11] = consts.IV0, consts.IV1, consts.IV2, consts.IV3
	s[12] = uint32(counter)
	s[13] = uint32(counter >> 32)
	s[14] = blen
	s[15] = flags

	rcompress(&s, block, out)
}
...
func rcompress(s *[16]uint32, m *[16]uint32, out *[16]uint32) {
	const (
		a = 10
		b = 11
		c = 12
