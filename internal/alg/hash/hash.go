...
import (
	...
	"github.com/zeebo/blake3/internal/alg/hash/hash_simd"
	"github.com/zeebo/blake3/internal/consts"
)

func HashF(input *[8192]byte, length, counter uint64, flags uint32, key *[8]uint32, out *[64]uint32, chain *[8]uint32) {
	if consts.HasSIMD && length > 2*consts.ChunkLen {
		hash_simd.HashF(input, length, counter, flags, key, out, chain)
...
func HashP(left, right *[64]uint32, flags uint32, key *[8]uint32, out *[64]uint32, n int) {
	if consts.HasSIMD && n >= 2 {
		hash_simd.HashP(left, right, flags, key, out, n)
