...
// transposeMsg gathers word w of sixteen chunks into the 64-byte slot at
// msg+64*w. The eight-lane ymm transpose is reused four times: chunk group g
// lands in the 32-byte half at +32*g, and j selects words 0-7 or 8-15.
// It runs on ymmAlloc (Y16-Y31) so it cannot alias the zmm state in Z0-Z15.
func transposeMsg(c Ctx, ymmAlloc *Alloc, block GPVirtual, input, msg Mem) {
	for j := 0; j < 2; j++ {
		for g := 0; g < 2; g++ {
			vs := ymmAlloc.Values(8)
			for i, v := range vs {
				VMOVDQU(input.Offset(1024*(8*g+i)+32*j).Idx(block, 1), v.Get())
			}
			transpose(c, ymmAlloc, vs)
			for i, v := range vs {
				VMOVDQU(v.Consume(), msg.Offset(64*(8*j+i)+32*g))
			}
		}
	}
}

// loadCounter splits sixteen 64-bit counters into a low and a high 32-bit lane
// vector. It reuses the eight-lane ymm sequence twice, writing the halves of the
// 64-byte destinations, so the counter never needs 512-bit integer ops.
func loadCounter(c Ctx, ymmAlloc *Alloc, mem, lo_mem, hi_mem Mem) {
	for g := 0; g < 2; g++ {
		base := 64 * g // eight counters per group, 8 bytes each

		ctr0, ctr1 := ymmAlloc.Value(), ymmAlloc.Value()
...
func xorb(alloc *Alloc, a, b *Value) *Value {
	...
	switch {
	case a.HasReg():
		VPXORD(b.ConsumeOp(), a.Consume(), o.Get())
	case b.HasReg():
		VPXORD(a.ConsumeOp(), b.Consume(), o.Get())
	default:
		VPXORD(a.ConsumeOp(), b.Consume(), o.Get())
	}
	return o
