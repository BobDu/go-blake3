...
func main() {
	...
	VMOVUPS(chain.Offset(0*16), rows[0])
	VMOVUPS(chain.Offset(1*16), rows[1])
	VMOVUPS(ivMem, rows[2])

	VPINSRD(U8(0), counter.As32(), rows[3], rows[3])
	SHRQ(U8(32), counter)
	...
	VMOVUPS(block.Offset(0*16), ms[0])
	VMOVUPS(block.Offset(1*16), ms[1])
	VMOVUPS(block.Offset(2*16), ms[2])
	VMOVUPS(block.Offset(3*16), ms[3])

	rot16, rot8 := XMM(), XMM()
	VMOVUPS(rot16Mem, rot16)
	VMOVUPS(rot8Mem, rot8)
	...
	VMOVUPS(rows[0], out.Offset(0*16))
	VMOVUPS(rows[1], out.Offset(1*16))
	VMOVUPS(rows[2], out.Offset(2*16))
	VMOVUPS(rows[3], out.Offset(3*16))
